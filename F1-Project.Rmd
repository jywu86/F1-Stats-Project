...
circuit <- transform(circuit, track_group = ifelse(Avg_Speed_MPH<127.6, 'Low_Speed',ifelse(Avg_Speed_MPH<152.4, 'Med_Speed','High_Speed')))

bw = 1
n_obs = sum(!is.na(circuit$Avg_Speed_MPH))

mean(circuit$Avg_Speed_MPH)
sd(circuit$Avg_Speed_MPH)

circuit %>% 
  ggplot(aes(x=Avg_Speed_MPH, color=track_group)) + 
  geom_histogram(binwidth=1) + 
  stat_function(fun=function(x) dnorm(x, mean=mean(circuit$Avg_Speed_MPH), sd = sd(circuit$Avg_Speed_MPH))* bw * n_obs)
